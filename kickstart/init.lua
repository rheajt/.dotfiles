...
do
  ...
  -- Use a known Python provider for Python-based Neovim plugins.
  -- Keep this Python environment updated with: python3.10 -m pip install --user --upgrade pynvim
...
--     defaults = {
--       mappings = {
--         i = { ['<c-enter>'] = 'to_fuzzy_refine', ['<C-q>'] = require('telescope.actions').smart_send_to_qflist },
--       },
--     },
--     -- pickers = {}
--     extensions = {
...
do
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      ...
      map('gdn', vim.diagnostic.goto_next, '[G]oto [D]iagnostic [N]ext')
      map('gdp', vim.diagnostic.goto_prev, '[G]oto [D]iagnostic [P]revious')
      map('gdf', vim.diagnostic.open_float, '[G]oto [D]iagnostic [F]loat')
