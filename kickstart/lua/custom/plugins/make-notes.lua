local function get_project_name()
  ...
  if vim.fn.filereadable(package_json) == 1 then
    ...
    if ok and type(decoded) == 'table' and type(decoded.name) == 'string' then return decoded.name end
  end
...
local function make_notes()
  ...
  if note ~= '' then
    ...
    if vim.fn.filereadable(today_notes) == 1 then lines = vim.fn.readfile(today_notes) end
...
vim.keymap.set('n', '<leader>md', function() print 'day over' end, { silent = true, desc = '[M]ake [D]ay over' })
